...
from wombat_msgs.action import WeedsSprayAction, WaypointAction, TurnAction
from geometry_msgs.msg import Point
from nav_msgs.msg import Odometry


WEEDS_SPRAY_ACTION = "weeds_spray_action"
WEED_SEPARATION_DIST = 0.10 # 0.1
...
class WeedsSprayActionServer(Node):
    ...
    def execute_callback(self, goal_handle):
        ...
        weed_positions = np.array(weed_positions).reshape(
            (len(weed_positions) // 2, 2))
        ...
        for i in range(weed_positions.shape[0]):
            ...
            dist_to_auxiliarly = dist_to_target - \
                (CHASSIS_WIDTH_M / 2 + WEED_SEPARATION_DIST)
            ...
            self.get_logger().info(
                f"Navigating to auxilliary point: {auxiliary_point}")
            waypoint_goal = WaypointAction.Goal()
            waypoint_goal.target = Point(
                x=auxiliary_point[0], y=auxiliary_point[1])
            self.waypoint_client.send_goal(waypoint_goal)
            self.get_logger().info("Reached auxiliary point")

            # rotate to face the weed before activating the sprayer
            rotate_goal = TurnAction.Goal()
            rotate_goal.target.data = np.arctan2(
                weed_pos[1] - self.current_position.y, weed_pos[0] - self.current_position.x)
